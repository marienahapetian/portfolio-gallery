		...
		<div id="portfolios-list-page">
			<form method="post" onkeypress="doNothing()" action="admin.php?page=portfolios_huge_it_portfolio" id="admin_form" name="admin_form">
				<h2>Huge-IT Portfolios
					<a onclick="window.location.href='<?php echo $add_new_portfolio_safe_link; ?>'" class="add-new-h2" ><?php echo __( 'Add New Portfolio', 'portfolio-gallery' );?></a>
				</h2>
				<?php
				$serch_value='';
				...
				<table class="wp-list-table widefat fixed pages" style="width:95%">
					...
					for ( $i = 0; $i < count( $rows ); $i ++ ) {
						...
						$delete_portfolio_safe_link = wp_nonce_url(
							'admin.php?page=portfolios_huge_it_portfolio&task=remove_cat&id=' . $rows[$i]->id,
						...
						<tr <?php if($trcount%2==0){ echo 'class="has-background"';}?>>
							...
							<td><a href="<?php esc_attr_e($edit_portfolio_safe_link); ?>"><?php echo esc_html(stripslashes($rows[$i]->name)); ?></a></td>
							<td>(<?php if(!($pr_count)){echo '0';} else{ echo $rows[$i]->prod_count;} ?>)</td>
							<td><a  href="<?php esc_attr_e($delete_portfolio_safe_link); ?>"><?php _e( 'Delete', 'portfolio-gallery' );?></a></td>
						</tr>
					<?php } ?>
					</tbody>
				</table>
